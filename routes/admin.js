...
router.post("/products-add", (req, res) => {
  ...
  productHelpers.addProduct(req.body, async (id) => {
    ...
    image.mv(
      "./public/product-imagesfull/" + id + "1" + ".jpg",
      (err, done) => {
        if (err) {
          ...
          compress_images(
            ...
            function (error, completed, statistic) {
              ...
              image.mv(
                "./public/product-imagesfull/" + id + "2" + ".jpg",
                (err, done) => {
                  if (err) {
                    productHelpers.deleteProduct(id).then(() => {
                      res.render("admin/productError", {
                    ...
                    compress_images(
                      ...
                      function (error, completed, statistic) {
                        ...
                        image.mv(
                          "./public/product-imagesfull/" + id + "3" + ".jpg",
                          (err, done) => {
                            if (err) {
                              productHelpers.deleteProduct(id).then(() => {
                                console.log(
                                  "deleteddddddddddddddddd2222222222222222"
                                );
                                ...
                                function (error, completed, statistic) {
                                  if (error) {
                                    productHelpers
                                      ...
                                      .then(() => {
                                        console.log(
                                          "deleteddddddddddddddddd3333333333333333"
                                        );
...
router.post("/edit-cakes", verifyLogin, async (req, res) => {
  ...
  productHelpers.updateProduct(req.body, proId).then(async () => {
    if (req.files) {
      ...
      if (image) {
        await image.mv(
          "./public/product-imagesfull/" + proId + "1" + ".jpg",
          (err, done) => {
            if (err) {
              res.render("admin/productError", { admin: true, adminlog: true });
            } else {
      ...
      if (image2) {
        image2.mv(
          "./public/product-imagesfull/" + proId + "2" + ".jpg",
          (err, done) => {
            if (err) {
              res.render("admin/productError", { admin: true, adminlog: true });
            } else {
      ...
      if (image3) {
        image3.mv(
          "./public/product-imagesfull/" + proId + "3" + ".jpg",
          (err, done) => {
            if (!err) {
...
router.get("/viewPendingOrders", verifyLogin, async (req, res) => {
  ...
  userHelper.getOrderDetails(req.query.id).then((response) => {
    ...
    res.render("admin/viewPendingOrders", {
      admin: true,
      orderdata: response,
      paymentmethod,
      adminName,
